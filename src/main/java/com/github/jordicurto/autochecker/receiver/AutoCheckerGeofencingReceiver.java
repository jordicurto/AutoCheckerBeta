...
public class AutoCheckerGeofencingReceiver extends BroadcastReceiver {
    ...
    @NonNull
    public static Intent createIntent(Context context, String action, Bundle bundle) {
    ...
    @NonNull
    public static Intent createIntent(Context context, String action, String param, long value) {
        Intent intent = createIntent(context, action);
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action != null) {
            ...
            switch (action) {
                case AutoCheckerConstants.INTENT_START_RECEIVER:
    ...
    private void handleCancelLeaveLocation(Context context) {
        ...
        mAutoCheckerAlarmsManager.cancelAlarmForceLeaveLocation();

        context.sendBroadcast(new Intent(AutoCheckerConstants.INTENT_ACTIVITY_RELOAD_REQUEST));
    }

    private void handleForceLeaveLocation(Context context, Intent intent) {
        ...
        if (duration > 0L) {
            ...
            mAutoCheckerAlarmsManager.setAlarmForceLeaveLocation(duration);
                            ...
                            switch (event.getGeofenceTransition()) {
                                case Geofence.GEOFENCE_TRANSITION_ENTER:
                                    ...
                                    if (location.getStatus() == WatchedLocation.OUTSIDE_LOCATION) {
                                        ...
                                        mAutoCheckerTransitionManager.
                                                cancelScheduledRegisterTransition(AutoCheckerTransitionManager.LEAVE_TRANSITION);
                                    }
                                    break;
                                case Geofence.GEOFENCE_TRANSITION_EXIT:
                                    if (location.getStatus() == WatchedLocation.INSIDE_LOCATION ||
                                            location.getStatus() == WatchedLocation.FORCED_OUTSIDE_LOCATION) {
                                        mAutoCheckerTransitionManager.
                                                scheduleRegisterTransition(location, time,
                                                        AutoCheckerTransitionManager.LEAVE_TRANSITION, delay);
                                    } else {
                                        mAutoCheckerTransitionManager.
                                                cancelScheduledRegisterTransition(AutoCheckerTransitionManager.ENTER_TRANSITION);
                                    }
                                    break;
                                default:
                                    break;
    ...
    private void handleConfirmTransition(Context context, Intent intent) {
        ...
        mAutoCheckerTransitionManager.registerTransition(location, time, direction);
