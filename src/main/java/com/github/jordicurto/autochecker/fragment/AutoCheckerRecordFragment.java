...
import com.github.jordicurto.autochecker.activity.AutoCheckerRecordsActivity;
 ...
 */
public class AutoCheckerRecordFragment extends Fragment {

    private static final String ARG_LOCATION_ID = "location_id";
    private static final String ARG_START_DATE_GRAPH = "start_date";
    private static final String ARG_END_DATE_GRAPH = "end_date";
    ...
    private AutoCheckerBusinessManager manager;
    ...
    public static AutoCheckerRecordFragment newInstance(String locationId, Date startDate, Date endDate) {
        AutoCheckerRecordFragment fragment = new AutoCheckerRecordFragment();
        Bundle args = new Bundle();
        args.putLong(ARG_START_DATE_GRAPH, startDate.getTime());
        args.putLong(ARG_END_DATE_GRAPH, endDate.getTime());
        args.putString(ARG_LOCATION_ID, locationId);
        fragment.setArguments(args);
        return fragment;
    }

    public AutoCheckerRecordFragment() {
        super();
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        manager = AutoCheckerBusinessManager.getManager(getContext());

        if (getArguments() != null) {

            location = manager.getWatchedLocation(getArguments().getString(ARG_LOCATION_ID));

            Date startDate = new Date(getArguments().getLong(ARG_START_DATE_GRAPH));
            Date endDate = new Date(getArguments().getLong(ARG_END_DATE_GRAPH));

            records = manager.getIntervalWatchedLocationRecord(location, startDate, endDate);

            startDay = startDate.getTime() / Duration.DAYS_PER_MILLISECOND;
            graphStart = startDate.getTime() % Duration.DAYS_PER_MILLISECOND;
        }

        colors[0] = getContext().getResources().getColor(R.color.colorOutLocation);
        colors[1] = getContext().getResources().getColor(R.color.colorInLocation);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View rootView = inflater.inflate(R.layout.fragment_auto_checker_record_usage_graph, container, false);

        charts.put(PART_OF_DAY.MORNING, (HorizontalBarChart) rootView.findViewById(R.id.graphMorning));
        charts.put(PART_OF_DAY.EVENING, (HorizontalBarChart) rootView.findViewById(R.id.graphEvening));
        charts.put(PART_OF_DAY.NIGHT, (HorizontalBarChart) rootView.findViewById(R.id.graphNight));
